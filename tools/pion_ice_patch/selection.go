...
func (s *controllingSelector) nominatePair(pair *CandidatePair) {
	...
	if os.Getenv("TUYA_RECORDINGS_ICE_SUPPRESS_USE_CANDIDATE") != "1" {
		setters = append(setters[:3], append([]stun.Setter{UseCandidate()}, setters[3:]...)...)
	}
	roleSetter := stun.Setter(AttrControlling(s.agent.tieBreaker))
	if os.Getenv("TUYA_RECORDINGS_FORCE_STUN_CONTROLLED") == "1" {
		roleSetter = AttrControlled(s.agent.tieBreaker)
	}
	setters = append(setters[:4], append([]stun.Setter{roleSetter}, setters[4:]...)...)
