...
func (a *Agent) startConnectivityChecks(isControlling bool, remoteUfrag, remotePwd string) error {
	...
	if os.Getenv("TUYA_RECORDINGS_FORCE_STUN_CONTROLLED") == "1" {
		isControlling = false
	}
...
func (a *Agent) sendBindingRequest(msg *stun.Message, local, remote Candidate) {
	...
	if os.Getenv("TUYA_RECORDINGS_LOG_STUN") == "1" {
		fmt.Fprintf(
			os.Stderr,
			"tuya_recordings_stun_out local=%q remote=%q username=%q controlling=%t controlled=%t useCandidate=%t priority=%d tx=%x\n",
			local.String(),
			remote.String(),
			stunUsername(msg),
			msg.Contains(stun.AttrICEControlling),
			msg.Contains(stun.AttrICEControlled),
...
func stunUsername(msg *stun.Message) string {
...
func stunPriority(msg *stun.Message) uint32 {
