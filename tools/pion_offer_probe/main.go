...
func main() {
	...
	replayDataChannelOnly := os.Getenv("TUYA_RECORDINGS_REPLAY_DATACHANNEL_ONLY") == "1"
	dummyAudioSendrecv := os.Getenv("TUYA_RECORDINGS_DUMMY_AUDIO_SENDRECV") == "1"
	if os.Getenv("TUYA_RECORDINGS_AUDIO_SENDRECV") == "1" {
		audioDirection = webrtc.RTPTransceiverDirectionSendrecv
	}
	if replayDataChannelOnly {
		// Some Tuya IPC replay paths use fMP4 over a data channel instead of RTP media tracks.
	} else if dummyAudioSendrecv {
		audioTrack, err := webrtc.NewTrackLocalStaticRTP(webrtc.RTPCodecCapability{
			MimeType:  webrtc.MimeTypePCMU,
			ClockRate: 8000,
		}, "audio", "tuya-recordings")
		if err != nil {
			log.Fatal(err)
		}
	...
	if replayDataChannelOnly || os.Getenv("TUYA_RECORDINGS_DATA_CHANNEL") == "1" {
		maxRetransmits := uint16(5)
		dataChannel, err = peer.CreateDataChannel("fmp4Stream", &webrtc.DataChannelInit{
	...
	trickleCandidates := os.Getenv("TUYA_RECORDINGS_TRICKLE_CANDIDATES") == "1"
	...
	if os.Getenv("TUYA_RECORDINGS_CHROME_SDP") == "1" {
...
func iceServersFromEnv() []webrtc.ICEServer {
	raw := os.Getenv("TUYA_RECORDINGS_ICE_SERVERS")
	if raw == "" {
		return nil
	}
	var configs []iceServerConfig
	if err := json.Unmarshal([]byte(raw), &configs); err != nil {
		log.Fatalf("invalid TUYA_RECORDINGS_ICE_SERVERS: %v", err)
	}
...
func iceTransportPolicyFromEnv() webrtc.ICETransportPolicy {
	switch strings.ToLower(strings.TrimSpace(os.Getenv("TUYA_RECORDINGS_ICE_TRANSPORT_POLICY"))) {
	case "relay":
...
func applyICESettingsFromEnv(settingEngine *webrtc.SettingEngine) {
	if os.Getenv("TUYA_RECORDINGS_ICE_LITE") == "1" {
		settingEngine.SetLite(true)
	}
	if os.Getenv("TUYA_RECORDINGS_DISABLE_MDNS") == "1" {
	...
	if minPort, maxPort, ok := udpPortRangeFromEnv(); ok {
		if err := settingEngine.SetEphemeralUDPPortRange(minPort, maxPort); err != nil {
			log.Fatalf("invalid TUYA_RECORDINGS_UDP_PORT_RANGE: %v", err)
		}
	}
}

func iceNetworkTypesFromEnv() []webrtc.NetworkType {
	raw := strings.TrimSpace(os.Getenv("TUYA_RECORDINGS_ICE_NETWORK_TYPES"))
	if raw == "" {
		return nil
	}
	...
	for _, item := range strings.Split(raw, ",") {
		...
		case "":
		default:
			log.Fatalf("invalid TUYA_RECORDINGS_ICE_NETWORK_TYPES entry: %s", item)
		}
	}
	return networkTypes
}
...
func udpPortRangeFromEnv() (uint16, uint16, bool) {
	raw := strings.TrimSpace(os.Getenv("TUYA_RECORDINGS_UDP_PORT_RANGE"))
	if raw == "" {
		return 0, 0, false
	}
	parts := strings.Split(raw, "-")
	if len(parts) != 2 {
		log.Fatalf("invalid TUYA_RECORDINGS_UDP_PORT_RANGE: %s", raw)
	}
	minPort, err := strconv.ParseUint(strings.TrimSpace(parts[0]), 10, 16)
	if err != nil {
		log.Fatalf("invalid TUYA_RECORDINGS_UDP_PORT_RANGE minimum: %v", err)
	}
	maxPort, err := strconv.ParseUint(strings.TrimSpace(parts[1]), 10, 16)
	if err != nil {
		log.Fatalf("invalid TUYA_RECORDINGS_UDP_PORT_RANGE maximum: %v", err)
	}
...
func runInteractive(peer *webrtc.PeerConnection, dataChannel *webrtc.DataChannel, offer string, timeout time.Duration) {
	...
	if dataChannel != nil {
		dataChannel.OnOpen(func() {
			...
			if os.Getenv("TUYA_RECORDINGS_DATA_CHANNEL_START_ON_OPEN") == "1" {
				_ = dataChannel.SendText("start")
			}
		})
	...
	if os.Getenv("TUYA_RECORDINGS_REMOTE_SDP_TYPE") == "pranswer" {
	...
	for {
		select {
		case <-deadline:
			writeResult(peer, tracks.Load(), packets.Load(), bytes.Load(), dataMessages.Load(), dataBytes.Load(), iceState.Load().(string), connectionState.Load().(string))
			return
		case <-ticker.C:
...
func rtpForwardersFromEnv() map[webrtc.RTPCodecType]net.Conn {
	...
	addForwarder(webrtc.RTPCodecTypeVideo, "TUYA_RECORDINGS_VIDEO_RTP_PORT")
	addForwarder(webrtc.RTPCodecTypeAudio, "TUYA_RECORDINGS_AUDIO_RTP_PORT")
	return forwarders
}

func h264FileFromEnv() *os.File {
	path := strings.TrimSpace(os.Getenv("TUYA_RECORDINGS_H264_OUTPUT"))
	if path == "" {
		return nil
	}
	file, err := os.Create(path)
	if err != nil {
		log.Fatalf("failed to open TUYA_RECORDINGS_H264_OUTPUT=%s: %v", path, err)
	}
	return file
}
...
func readCandidates(peer *webrtc.PeerConnection, scanner *bufio.Scanner) {
	for scanner.Scan() {
		...
		if sdpMid == "" {
			sdpMid = strings.TrimSpace(os.Getenv("TUYA_RECORDINGS_REMOTE_CANDIDATE_MID"))
		}
		if sdpMid == "" {
			sdpMid = "0"
		}
...
func writeResult(peer *webrtc.PeerConnection, tracks uint64, packets uint64, bytes uint64, dataMessages uint64, dataBytes uint64, iceState string, connectionState string) {
	if err := writeJSON(map[string]any{
		"type":            "result",
		"tracks":          tracks,
		"packets":         packets,
